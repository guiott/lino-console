...
{// GPS & IMU (DCM) parameters
  ...
  UDB4.pitchDeg = UDB4.pitchRad / (2*Math.PI) * 360;
  
  UDB4.rollRad = Math.atan2(- UDB4.rmat[6] , UDB4.rmat[8]);
  UDB4.rollDeg = UDB4.rollRad / (2*Math.PI) * 360;
  
  UDB4.yawRad = Math.atan2(- UDB4.rmat[1] , UDB4.rmat[4]);
  UDB4.yawDeg = (UDB4.yawRad / (2 * Math.PI)) * 360; 
...
{// values coming from Low Level Supervisor
  ...
  LLS.temp[0] = linBuff.readInt8(2); // Internal skulls temperature
  LLS.temp[1] = linBuff.readInt8(3); 
  
  // obstacle distance @ x degrees 
  // (http://www.guiott.com/QuadSonar/HR/LinoSonar.png)
  LLS.obst[0] = linBuff.readInt8(4);  // LL Lef Left -52°
  LLS.obst[1] = linBuff.readInt8(5);  // LC Left Center -27°
  LLS.obst[2] = linBuff.readInt8(6);  // CL Center Left -12°
  LLS.obst[3] = linBuff.readInt8(7);  // CC Center Center 0°
  LLS.obst[4] = linBuff.readInt8(8);  // CR Center Right 12°
  LLS.obst[5] = linBuff.readInt8(9);  // RC Right Center 27°
  LLS.obst[6] = linBuff.readInt8(10); // RR Right Right 52°
  LLS.lPwrOff = linBuff.readInt8(11);  // LLS -> HLS switch off. 0 = PowerOn, 1 = PowerOff
